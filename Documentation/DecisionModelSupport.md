...
- **Timeouts and cancellation end the session.** They are set by `startupTimeout` (default 600 s) and `requestTimeout` (default 300 s). The request deadline covers both writing the request and waiting for the reply: a worker that stops reading stdin is stopped (SIGTERM, then SIGKILL) so a blocked write fails with `.timedOut`, and cancelling during the write stops the worker promptly and throws `CancellationError`. Afterwards `isClosed` is true and later calls throw `.closed`; start a new session. `FluidUseOfficialBench run` does this automatically, recording the failed row first and timing each restart separately (`worker_restarts` in its meta file).
