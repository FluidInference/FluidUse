...
Product issue confirmed: on this in-domain data the shipped per-K calibrator makes `answer`'s probabilities markedly under-confident (ECE 21.2% vs 1.9% uncalibrated), and the app rendering costs 1.9 pt accuracy against the author's evaluation rendering. `VerdictManager` applies the calibrator exactly as the author ships it; whether to change the default (e.g. expose T=1 / a refit) is a product decision that needs a regression test on this data before any change.
