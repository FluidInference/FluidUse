...
(`cua-s1-forms/coreml/reports`, `laya/coreml/reports`) and, for laya, in
[Benchmarks/laya](Benchmarks/laya) in this repository.
...
```bash
# laya, from this repository (downloads the buckets on first run)
swift run -c release FluidUseLaya benchmark --suites Benchmarks/laya/suites.jsonl \
    --reference Benchmarks/laya/reference-rows.jsonl --report /tmp/laya.json
swift run -c release FluidUseLaya benchmark --suites Benchmarks/laya/suites.jsonl \
