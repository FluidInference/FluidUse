    ...
    private static func printUsage() {
        print(
            ...
            Usage: swift run FluidUseLaya benchmark --suites suites.jsonl [--reference reference-rows.jsonl]
                                                [--model-dir DIR] [--precision fp16|e8] [--lengths 128,256,512,1024] [--only suite,…]
                                                [--limit N] [--report out.json]

            Suites and reference rows come from mobius models/computer-use/laya/coreml/benchmark (benchmark.py).
            """)
    }
}
